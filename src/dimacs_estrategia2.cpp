...
int partition_sum(Graph& g, ClusterGraph& cg, SList<node>* partition, int& nClusters, NodeArray<NodeArray<int>>& dist)
{
    int sum = 0;
	for(int i = 0; i < nClusters - 1; ++i)
	{
		for(SListIterator<node> iter_i = partition[i].begin(); iter_i.valid(); iter_i = iter_i.succ())
		{
			for(int j = i + 1; j < nClusters; j++)
			{
				for(SListIterator<node> iter_j = partition[j].begin(); iter_j.valid(); iter_j = iter_j.succ())
				{
					sum += dist[*iter_i][*iter_j];
				}
			}
		}
	}
...
int main(int argc, char *argv[])
{
	...
	printf("\nbest fitness: %.4lf\n", partition_sum(G, cg, partition, k, dist) / (double)10 );
